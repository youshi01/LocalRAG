...
不要求使用本机 Ollama，可以配置公网、公司内网或其他 OpenAI Compatible 服务。
Windows Docker 开发环境中，后端访问宿主机 Ollama 使用
`http://host.docker.internal:11434`；Windows 本机直接运行后端使用
`http://localhost:11434`。本项目中期演示采用：
